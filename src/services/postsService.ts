...
export const postsService = {
  ...
  async getLikes(id: PostId, page = 1, limit = 20) {
    const res = await api.get(`/posts/${id}/likes`, { params: { page, limit } });
    return res.data;
  },
  ...
  async getUserPosts(username: string, page = 1, limit = 20) {
    const res = await api.get(`/users/${username}/posts`, { params: { page, limit } });
    return res.data;
  },

  async getUserLikes(username: string, page = 1, limit = 20) {
    const res = await api.get(`/users/${username}/likes`, { params: { page, limit } });
    return res.data;
  },
};
