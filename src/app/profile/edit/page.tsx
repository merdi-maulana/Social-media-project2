...
export default function EditProfilePage() {
  ...
  const handleSubmit = () => {
    mutation.mutate({ name, username, email, phone, bio, avatarFile });
  };
  ...
  const inputClass =
    "w-full bg-neutral-900 border border-white/10 rounded-xl px-4 py-3 text-white text-sm outline-none focus:border-purple-500 transition-colors";
    ...
    <>
        ...
        <div className="max-w-xl mx-auto px-4 h-14 flex items-center justify-between">
          ...
          {avatarPreview ? (
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 p-[2px]">
              <img src={avatarPreview} alt={username} className="w-full h-full rounded-full object-cover" />
            </div>
          ) : (
      ...
      <main className="max-w-xl mx-auto px-4 pb-12">
        ...
        <div className="flex flex-col items-center pt-8">
          ...
          <input ref={fileRef} type="file" accept="image/png,image/jpeg,image/webp" onChange={handleAvatarChange} className="hidden" />
          <button
        ...
        <div className="mt-8 space-y-5">
          ...
          <div>
            ...
            <input type="email" value={email} readOnly className="w-full bg-neutral-900/50 border border-white/5 rounded-xl px-4 py-3 text-gray-400 text-sm outline-none cursor-not-allowed" />
          </div>
          <div>
            <label className="block text-white text-sm font-semibold mb-2">Number Phone</label>
