...
import type { CreatePostErrors } from "./type";
...
export default function CreatePostPage() {
  ...
  const handleFile = useCallback((file: File) => {
    if (!file.type.match(/^image\/(png|jpeg|webp)$/)) {
      setErrors((e) => ({ ...e, image: "Only PNG, JPG, or WEBP files are allowed" }));
      return;
    }
  ...
  return (
    <>
      {/* Toast */}
      {toast && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[200] px-6 py-3 rounded-full bg-green-600 text-white text-sm font-medium shadow-lg shadow-green-500/30 flex items-center gap-2">
          {toast}
      ...
      <main className="max-w-xl mx-auto px-4 pb-12">
        ...
        <div className="mt-6">
          ...
          {imagePreview ? (
            <div className="rounded-xl overflow-hidden bg-neutral-900 border border-white/10">
              ...
              <div className="flex gap-2 p-3">
                <button onClick={() => fileRef.current?.click()} className="flex items-center gap-2 px-4 py-2 rounded-full bg-neutral-800 text-white text-sm font-medium hover:bg-neutral-700 transition-colors border border-white/10">
                  <ArrowUpFromLine className="h-4 w-4" /> Change Image
                </button>
                <button onClick={removeImage} className="flex items-center gap-2 px-4 py-2 rounded-full bg-neutral-800 text-red-400 text-sm font-medium hover:bg-red-500/10 transition-colors border border-white/10">
          ...
          {errors.image && <p className="mt-2 text-red-400 text-xs">{errors.image}</p>}
        ...
        <div className="mt-6">
          ...
          <textarea
            ...
            className={`w-full bg-neutral-900 rounded-xl px-4 py-3 text-white text-sm outline-none resize-none transition-colors border ${
              errors.caption ? "border-red-500" : "border-white/10 focus:border-purple-500"
            }`}
          />
          {errors.caption && <p className="mt-2 text-red-400 text-xs">{errors.caption}</p>}
        </div>
        ...
        {mutation.isError && (
          <p className="mt-3 text-center text-red-400 text-sm">{extractErrorMessage(mutation.error)}</p>
        )}
      </main>
    </>
  );
